...
<html class="loading" lang="@if(session()->has('locale')){{session()->get('locale')}}@else{{$configData['defaultLanguage']}}@endif"
 data-textdirection="{{$configData['direction'] == 'rtl' ? 'rtl' : 'ltr' }}" data-asset-path="{{ asset('/')}}">
  <!-- BEGIN: Head-->
    ...
    <title>@yield('title') - TranDanh template</title>
    <link rel="apple-touch-icon" href="{{asset('images/ico/logo-dulich.png')}}">
    <link rel="shortcut icon" type="image/x-icon" href="{{asset('images/ico/logo-dulich.png')}}">
